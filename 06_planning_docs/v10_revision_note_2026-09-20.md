...
용어 첫 정의(DTI, ZIP3, BLS, TF-IDF, AUC, CatBoost), 60단어 안밖 문장 분할, 지시어 모호성, "2008–2014" 표본 수 표현, "recovery sample" 표현 정정 등.
...
- E2 H2 하향 강도: "loses precision" → "크기는 유지되나 0과 구분되지 않아, 종교적 표현을 나머지 텍스트와 분리할 수 없다". 초록·결론·5.2절 판정 문장·표 9·커버레터 동일 표현으로 통일. 점추정치가 거의 변하지 않는다는 사실은 함께 명시.
